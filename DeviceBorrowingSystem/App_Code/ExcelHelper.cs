...
/// </summary>
    ...
    public class MyExcel
    {
        ...
        /// <param name="nCol"></param>
        /// <param name="interior"></param>
        public void SetCellbackgroundStyle(int nRow, int nCol, int colorIndex)
        {
            Range rg = (Range)_MyRange.get_Item(nRow, nCol);
            rg.Interior.ColorIndex = colorIndex;
        }
        ...
        public void SetBordersLineStyle(int nRow, int nCol, LineStyle lineStyle)
        {
            ...
            rg.BorderAround2(lineStyle, Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin, Microsoft.Office.Interop.Excel.XlColorIndex.xlColorIndexAutomatic, Type.Missing, Type.Missing);
        }
        /// <summary>
        /// 设置表格区域边框
        /// </summary>
        ...
        public void MergeCells(int sRow, int sCol, int eRow, int eCol)
        {

            Range rg = _MyRange.get_Range(_MyRange.Cells[sRow, sRow], _MyRange.Cells[eRow, eCol]);
            rg.Merge(Type.Missing);
            //GetRange(strBegin, strEnd);
            //_MyRange.Merge(Type.Missing);
        }
        ...
        {
            Range rg = _MyWorksheet.get_Range(cell, Type.Missing);
        ...
        public void SetOrientation(int nRow, int nCol, int angle)
        {
            Range rg = (Range)_MyRange.get_Item(nRow, nCol);
            rg.Orientation = angle;
        }
        public void WrapText(int nRow, int nCol)
        {
            Range rg = (Range)_MyRange.get_Item(nRow, nCol);
            rg.WrapText = true;
        }
        ...
        public void AddPicture(int row, int col, string picturePath, int width, int height)
        {
            Range rg = (Range)_MyWorksheet.Cells[row, col];
            rg.Select();
            float PicLeft = Convert.ToSingle(rg.Left);
        ...
        public bool AddCol(int ColIndex)
        {
            if (_MyWorksheet != null)
            {
                Excel.Range rg = (Range)_MyWorksheet.Columns[ColIndex, Type.Missing];
                rg.Insert(XlDirection.xlToRight, Missing.Value);
                return true;
            }
            return false;
        }
        ...
        public void SetHyperLink(string strBegin, string strEnd, string sheetName, string CellStr)
        {
            ...
            rg.Hyperlinks.Add(rg, hlStr, Type.Missing, Type.Missing, Type.Missing);
        }
        /// <summary>
        /// 添加备注
        /// </summary>
        ...
        public void InsertComment(int nRow, int nCol, string comment)
        {
            ...
            {
                string commentStr = rg.Comment.Text(Type.Missing, Type.Missing, Type.Missing);
                commentStr += "\r\n" + comment;
     ...
     */
        public static string GetLetter(int columnNum)
        {
            int first;
            int last;
            string result = "";
            ...
            if (first > 0)
                result = Convert.ToChar(first + 64).ToString();

            if (last > 0)
                result = result + Convert.ToChar(last + 64).ToString();
